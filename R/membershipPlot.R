# the aesthetics of color choice are important here
library(RColorBrewer)

#################################################
#
# membershipPlot(): Plot a series of bars or pies indicating membership
#                   in groups with colors and bar/slice sizes.
#
# tab is a matrix of counts or proportions for observations of group membership (columns)
# within sites (rows).  The table is standardized by site so it doesn't matter whether the
  ...
  if (method == "bar") {
    ...
    if (to.file) {
      ...
      par(mar=c(x.mar.width,3,3,0), mgp=c(1.9,0.4,0), 
          bty="n", xpd=NA, las=label.rotation, tcl=-0.25, cex=cex)
    } else {
    ...
    if (! is.null(l1))
      text(xl, 1.250, labels=l1, cex=header.cex)
    if (! is.null(l2))
    ...
    par(mfrow=c(n.pie.row, n.pie.col), mar=c(1,1,1,1), bty="n", xpd=NA, cex=cex)
    for (p in 1:n.pie) {
...
#################################################
#
# The original version of membershipPlot()

membershipPlot.v0 <- function(tab, 
                           method=c("bar","pie"), 
                           fill.method=c("bw","color"),
                           distinguish.multiton=FALSE,
                           postscript.file=NULL, 
